...
public class FennecFoxBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("fennec_fox_baby");
    public static final Model MODEL = FennecFox.MODEL;
    private final EntityHolder<FennecFoxBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 6.0).add(Attributes.MOVEMENT_SPEED, 0.3);
    }

    @Override public EntityHolder<FennecFoxBaby> getHolder() { return this.holder; }

    public FennecFoxBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level);
    ...
    @Override protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof FennecFox));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.FENNEC_FOX, babyAge, ageFrozen); }
    ...
    @Override protected void dropCustomDeathLoot(@NotNull ServerLevel l, @NotNull DamageSource s, boolean r) { super.dropCustomDeathLoot(l, s, r); if (this.random.nextInt(4) == 0) this.spawnAtLocation(l, new ItemStack(net.minecraft.world.item.Items.RABBIT_HIDE, 1)); }
