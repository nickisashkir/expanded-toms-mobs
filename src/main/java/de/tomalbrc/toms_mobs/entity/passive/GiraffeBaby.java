...
public class GiraffeBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("giraffe_baby");
    public static final Model MODEL = Util.loadBbModel(ID);
    private final EntityHolder<GiraffeBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 15.0).add(Attributes.MOVEMENT_SPEED, 0.25);
    }
    @Override public EntityHolder<GiraffeBaby> getHolder() { return this.holder; }
    public GiraffeBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level); this.holder = new LivingEntityHolder<>(this, MODEL); EntityAttachment.ofTicking(this.holder, this);
    }
    @Override protected void registerGoals() {
        this.goalSelector.addGoal(0, new FloatGoal(this));
        this.goalSelector.addGoal(1, new PanicGoal(this, 1.4));
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof Giraffe));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.GIRAFFE, babyAge, ageFrozen); }
