...
public class ReindeerBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("reindeer_baby");
    public static final Model MODEL = Reindeer.MODEL;
    private final EntityHolder<ReindeerBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 12.0).add(Attributes.MOVEMENT_SPEED, 0.25);
    }

    @Override public EntityHolder<ReindeerBaby> getHolder() { return this.holder; }

    public ReindeerBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level);
    ...
    @Override protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof Reindeer));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.REINDEER, babyAge, ageFrozen); }
    ...
    @Override protected void dropCustomDeathLoot(@NotNull ServerLevel l, @NotNull DamageSource s, boolean r) { super.dropCustomDeathLoot(l, s, r); if (this.random.nextInt(4) == 0) this.spawnAtLocation(l, new ItemStack(net.minecraft.world.item.Items.LEATHER, 1)); }
