...
public class HedgehogBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("hedgehog_baby");
    public static final Model MODEL = Hedgehog.MODEL;
    private final EntityHolder<HedgehogBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 5.0).add(Attributes.MOVEMENT_SPEED, 0.2);
    }

    @Override public EntityHolder<HedgehogBaby> getHolder() { return this.holder; }

    public HedgehogBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level);
        this.holder = new LivingEntityHolder<>(this, MODEL);
        this.holder.setScale(0.6f);
    ...
    @Override protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof Hedgehog));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.HEDGEHOG, babyAge, ageFrozen); }
        if (this.tickCount % 2 == 0) { AnimationHelper.updateWalkAnimation(this, this.holder); AnimationHelper.updateHurtColor(this, this.holder); }
    }
    ...
    @Override protected void dropCustomDeathLoot(@NotNull ServerLevel l, @NotNull DamageSource s, boolean r) { super.dropCustomDeathLoot(l, s, r); if (this.random.nextInt(4) == 0) this.spawnAtLocation(l, new ItemStack(net.minecraft.world.item.Items.APPLE, 1)); }
    @Override protected SoundEvent getHurtSound(@NotNull DamageSource d) { return SoundEvents.FOX_SNIFF; }
