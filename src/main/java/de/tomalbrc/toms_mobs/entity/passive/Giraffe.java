...
public class Giraffe extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("giraffe");
    public static final Model MODEL = Util.loadBbModel(ID);
    private final EntityHolder<Giraffe> holder;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 40.0).add(Attributes.MOVEMENT_SPEED, 0.22);
    }
    @Override public EntityHolder<Giraffe> getHolder() { return this.holder; }
    public Giraffe(EntityType<? extends @NotNull Animal> type, Level level) {
    ...
    @Override public boolean isFood(ItemStack i) { return i.is(Items.HAY_BLOCK) || i.is(Items.APPLE); }
    @Nullable @Override public AgeableMob getBreedOffspring(@NotNull ServerLevel l, @NotNull AgeableMob m) { return new GiraffeBaby(MobRegistry.GIRAFFE_BABY, l); }
