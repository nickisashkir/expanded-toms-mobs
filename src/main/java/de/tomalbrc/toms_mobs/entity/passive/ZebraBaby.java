...
public class ZebraBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("zebra_baby");
    public static final Model MODEL = Util.loadBbModel(ID);
    private final EntityHolder<ZebraBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;
    @NotNull public static AttributeSupplier.Builder createAttributes() { return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 10.0).add(Attributes.MOVEMENT_SPEED, 0.35); }
    @Override public EntityHolder<ZebraBaby> getHolder() { return this.holder; }
    public ZebraBaby(EntityType<? extends @NotNull Animal> type, Level level) { super(type, level); this.holder = new LivingEntityHolder<>(this, MODEL); EntityAttachment.ofTicking(this.holder, this); }
    @Override protected void registerGoals() {
        this.goalSelector.addGoal(0, new FloatGoal(this));
        this.goalSelector.addGoal(1, new PanicGoal(this, 1.6));
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof Zebra));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.ZEBRA, babyAge, ageFrozen); }
