...
public class MeerkatBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("meerkat_baby");
    public static final Model MODEL = Meerkat.MODEL;
    private final EntityHolder<MeerkatBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 10.0).add(Attributes.MOVEMENT_SPEED, 0.3);
    }

    @Override public EntityHolder<MeerkatBaby> getHolder() { return this.holder; }

    public MeerkatBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level);
    ...
    @Override protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof Meerkat));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.MEERKAT, babyAge, ageFrozen); }
