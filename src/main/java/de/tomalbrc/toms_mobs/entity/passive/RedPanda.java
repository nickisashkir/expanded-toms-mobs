...
public class RedPanda extends TamableAnimal implements AnimatedEntity {
    ...
    @Override
    public EntityHolder<RedPanda> getHolder() { return this.holder; }

    public RedPanda(EntityType<? extends @NotNull TamableAnimal> type, Level level) {
        super(type, level);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new FollowOwnerGoal(this, 1.0, 10.0F, 2.0F) {
            @Override public boolean canUse() { return !RedPanda.this.isIndependent() && super.canUse(); }
        });
    ...
    public void tick() {
        ...
        // Trigger scared pose when a fight is happening nearby
        if (!this.level().isClientSide() && this.tickCount % 20 == 0 && scaredTicks <= 0 && !this.isOrderedToSit()) {
            if (detectNearbyCombat()) scaredTicks = 60;
        }
    ...
    private boolean detectNearbyCombat() {
        ...
        for (Player p : this.level().getEntitiesOfClass(Player.class, area)) {
            // Player attacked something within the last 60 ticks
    ...
    public InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (this.isOwnedBy(player) && player.isSecondaryUseActive() && itemInHand.isEmpty()) {
            ...
            if (independent) {
                ...
                this.setOrderedToSit(false);
                this.setInSittingPose(false);
                if (player instanceof net.minecraft.server.level.ServerPlayer sp) sp.sendSystemMessage(net.minecraft.network.chat.Component.literal("Red Panda is now independent and will wander here."), true);
                ...
                if (player instanceof net.minecraft.server.level.ServerPlayer sp) sp.sendSystemMessage(net.minecraft.network.chat.Component.literal("Red Panda will follow you again."), true);
            }
    ...
    @Override
    public boolean isFood(ItemStack itemStack) { return itemStack.is(Items.BAMBOO); }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(@NotNull ServerLevel level, @NotNull AgeableMob mate) {
        return new RedPandaBaby(MobRegistry.RED_PANDA_BABY, level);
    }
