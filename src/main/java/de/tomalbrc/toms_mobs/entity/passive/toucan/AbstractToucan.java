...
public abstract class AbstractToucan extends Animal implements AnimatedEntity {
    private final EntityHolder<? extends AbstractToucan> holder;
    ...
    @Override public EntityHolder<? extends AbstractToucan> getHolder() { return this.holder; }

    protected AbstractToucan(EntityType<? extends Animal> type, Level level, Model model) {
        super(type, level);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new de.tomalbrc.toms_mobs.entity.goal.flying.BirdPerchGoal(this, 1.0, 40, 1600, 4000));
    ...
    public void tick() {
        ...
        // Call at dawn (ticks 22500-500) occasionally
        if (!this.level().isClientSide() && this.tickCount % 40 == 0 && callTicks <= 0) {
            ...
            if (isDawn && this.random.nextInt(40) == 0) {
                callTicks = 40;
                this.playSound(SoundEvents.PARROT_AMBIENT, 0.7F, 0.9F + this.random.nextFloat() * 0.3F);
            }
        }
    ...
    @Override public boolean isFood(ItemStack i) { return i.is(Items.MELON_SLICE) || i.is(Items.SWEET_BERRIES) || i.is(Items.APPLE); }
