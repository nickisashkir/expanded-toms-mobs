...
public class RedPandaBaby extends Animal implements AnimatedEntity {
    public static final Identifier ID = Util.id("red_panda_baby");
    public static final Model MODEL = RedPanda.MODEL;
    private final EntityHolder<RedPandaBaby> holder;
    private int babyAge = 0;
    private boolean ageFrozen = false;

    @NotNull public static AttributeSupplier.Builder createAttributes() {
        return Animal.createMobAttributes().add(Attributes.MAX_HEALTH, 8.0).add(Attributes.MOVEMENT_SPEED, 0.25);
    }

    @Override public EntityHolder<RedPandaBaby> getHolder() { return this.holder; }

    public RedPandaBaby(EntityType<? extends @NotNull Animal> type, Level level) {
        super(type, level);
    ...
    @Override protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new FollowAdultGoal(this, 1.0, a -> a instanceof RedPanda));
    ...
    @Override public void tick() {
        super.tick();
        if (!this.level().isClientSide()) { babyAge++; BabyGrowthHelper.tryGrowUp(this, MobRegistry.RED_PANDA, babyAge, ageFrozen); }
